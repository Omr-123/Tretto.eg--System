...
<div class="card">
    <div class="card-head">
        <div class="card-title">Exchange Requests</div>
    </div>
    <table>
        <thead>
            <tr>
        ...
        <tbody>
            <?php foreach ($exchanges as $e):
                $id = (int) ($e['exchangeID'] ?? 0);
                $status = $e['status'] ?? 'Pending'; ?>
                <tr>
                    <td>#<?= $id ?></td>
                    <td><?= h($e['userName'] ?? $e['userID'] ?? '') ?></td>
                    <td><?= h($e['oldProductID'] ?? $e['OldProductID'] ?? '') ?></td>
                    <td><?= h($e['newProductID'] ?? $e['NewProductID'] ?? '') ?></td>
                    <td><?= h($e['reason'] ?? '') ?></td>
                    <td><span class="badge badge-<?= h(strtolower($status)) ?>"><?= h($status) ?></span></td>
                    <td><button class="btn btn-rose" onclick="applyEx(<?= $id ?>)">Approve</button><button
                            class="btn btn-danger" onclick="denyEx(<?= $id ?>)">Deny</button></td>
                </tr><?php endforeach; ?>
            <?php if (!$exchanges): ?>
                <tr>
                    <td colspan="7" style="text-align:center;color:var(--muted)">No exchanges found.</td>
                </tr><?php endif; ?>
        </tbody>
    </table>
</div>
